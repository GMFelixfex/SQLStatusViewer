...
var currentHost = "http://localhost:8100/";
...
function AddSource() {
    ...
    var UTCTime = document.getElementById("DataSourceUTCTime");
...
function GenerateSidepanel() {
    ...
    for (let index = 0; index < CurrentServerObjects.length; index++) {
        ...
        if (typeof (CurrentServerObjects[index].Data) === "string")
            e.innerHTML = "<a href='#'><i class='uil-exclamation-octagon fa-fw'></i>" + CurrentServerObjects[index].SourceObject.DisplayTitle + "</a>";
        else
...
function GenerateStatus() {
    ...
    if (gridsection != null && listsection != null) {
        ...
        if (_method == "Standart") {
            ...
            var forSorting = [];
            for (let i = 0; i < CurrentServerObjects.length; i++) {
                if (CurrentServerObjects[i].Data instanceof String)
                    var datetouse = "";
                else
...
function DisplayAs(_currentobject, _displaytype, _method, _gridbox, _listbox) {
    if (_currentobject.SourceObject.DisplayTitle.toLowerCase().includes(currentSearchTerm) || currentSearchTerm == "") {
        ...
        if (_displaytype == "Grid") {
            ...
            if (errorDisplay)
                e.setAttribute("class", "d-flex align-items-center rounded-2 p-3 Statusdiv StatusDivError");
            else
            ...
            var e = document.createElement("div");
            if (errorDisplay)
                e.setAttribute("class", "d-flex align-items-center rounded-2 p-2 Statusdiv2 StatusDivError");
            else
...
function FormatDateTime(_dateTime, _dateFormat) {
    console.log(_dateFormat);
    console.log(utcOffsets);
    if (!Number.isNaN(Date.parse(_dateTime))) {
        let originalDate = new Date(Date.parse(_dateTime));
...
function StartSetupUTCTime() {
    ...
    var UTCSelectBox = document.getElementById("UTCSelect");
    ...
    UTCSelectBox.addEventListener("change", function () {
        var selectElement = document.getElementById("UTCSelect");
        if (selectElement != null) {
            console.log(selectElement.value);
            GenerateStatus();
        }
...
function ResetUTCLabel() {
    var DataSourceUTCSelectBox = document.getElementById("DataSourceUTCTime");
